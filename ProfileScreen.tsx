...
import { ConditionalBlur as BlurView } from '@/components/ConditionalBlur';
...
import { AchievementProgress, Chat } from '@/lib/types';
...
export default function ProfileScreen() {
  ...
  const [friendsTab, setFriendsTab] = useState<'suggestions' | 'requests' | 'categories' | 'chats' | 'location'>('suggestions');
  ...
  const { createChat, loadUserChats } = useMessagesStore();

  const handleChatPress = async (friendId: string, friendName: string) => {
    try {
      const chatData = {
        is_private: true,
        participant_ids: [parseInt(friendId)]
      };
      const chat = await createChat(chatData);
      // refresh chat list
      await loadUserChats();
      navigation.navigate('Chat', { chatId: chat.id, friendName });
    } catch (error) {
  ...
  const handleOpenChat = (chat: Chat) => {
    ...
    const participant = chat.participants?.find(p => p.id !== (user?.id || '')) || chat.participants?.[0];
    const friendName = (participant as any)?.name || (participant as any)?.username || 'Чат';
    navigation.navigate('Chat', { chatId: chat.id, friendName });
  };
  ...
  const handleAchievementPress = async (achievement: AchievementProgress) => {
    console.log('🎯 Нажатие на достижение:', achievement.achievement.title);
    console.log('📊 Achievement data:', achievement);

    // Обновляем данные достижения перед открытием модального окна
    try {
  ...
  return (
                ...
                <View style={styles.nameSection}>
                  {isEditing ? (
                    ...
                    <>
                      ...
                      {user?.bio && (
                        <Text style={[styles.userBio, { color: colors.text3 }]}>
                          {user.bio}
                        </Text>
                      )}
                      {user?.phone && (
                        <View style={styles.userContact}>
                          ...
                          <Text style={[styles.userContactText, { color: colors.text3 }]}>
                            {user.phone}
                          </Text>
                        </View>
                      )}
          ...
          <View style={{ marginTop: Spacing.lg }}>
            ...
            <View style={styles.friendsTabs}>
              ...
              <TouchableOpacity
                style={[
                  styles.friendsTab,
                  friendsTab === 'chats' && styles.friendsTabActive,
                ]}
                onPress={() => setFriendsTab('chats')}
              >
                <Ionicons
                  name="chatbubbles"
                  size={16}
                  color={friendsTab === 'chats' ? colors.accent : colors.text3}
                />
                <Text
                  style={[
                    styles.friendsTabText,
                    friendsTab === 'chats' && styles.friendsTabTextActive,
                    { color: friendsTab === 'chats' ? colors.accent : colors.text3 },
                  ]}
                  numberOfLines={1}
                >
            ...
            {friendsTab === 'categories' && (
              <FriendCategoriesList 
                onProfilePress={handleFriendPress}
                onChatPress={handleChatPress}
              />
            )}
            {friendsTab === 'chats' && (
              <ChatsList onChatPress={handleOpenChat} />
            )}
            {friendsTab === 'location' && (
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: colors.backgroundSecondary, borderColor: colors.glassBorder, borderWidth: 1 }]}>
          {selectedAchievement && (
            <>
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    ...
    paddingBottom: 120, // Увеличен отступ под навигацию
    gap: Spacing.xl,
  },
  ...
  avatar: {
    ...
    shadowRadius: 16,
    elevation: 8,
    overflow: 'hidden', // Для правильного отображения изображений
  },
  avatarText: {
    fontSize: 36,
    color: '#2B1F05',
